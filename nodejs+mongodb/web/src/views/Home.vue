    ...
    <div class="mb-7">
      <div class="bgImg d-flex jc-center" id="header">
        <div class="text-white d-flex jc-center ai-center">
          <span class="shouye-text">Live football, play football！</span>
    ...
    <div class="post-container d-flex flex-wrap jc-center ai-center">
      <div v-for="(article, i) in articles" :key="i">
        <div class="show home-art">
          <router-link tag="div" :to="`/article/list/${article._id}`" class="top mt-10 mx-8 mb-0 hand"
            :style="{ 'background-image': `url(${article.icon})`, 'width': 'auto' }"></router-link>
          <div class="bg-postcolor pt-8 mx-8 title">
            <router-link tag="span" :to="`/article/list/${article._id}`"
    ...
    <div class="my-10" v-if="articles.length > 0">
      <div class="page-navigator d-flex jc-center">
        <div :class="{ current: 1 == pagination.currentPage }" class="mx-4 hand fs-md">
          <a @click="goToPage(1)" data-hover="home">
            <span class="text-grey-1">Home</span>
          </a>
        </div>
        ...
        <div class="mx-4 hand fs-md" :class="{ current: article == pagination.currentPage }"
          v-for="article in pagination.totalPage" :key="article">
          <a @click="goToPage(article)" :data-hover="article" class="text-grey-1">
            {{ article }}
          </a>
        </div>
        ...
        <div class="mx-4 hand fs-md" :class="{ current: pagination.totalPage == pagination.currentPage }">
          <a @click="goToPage(pagination.totalPage)" data-hover="Last">
            <span class="text-grey-1">Last</span>
          </a>
        </div>
        <div class="current mx-4 fs-md">
          <span class="text-grey-1">
            Section {{ pagination.currentPage }} Page / Total {{ pagination.totalPage }} Page
          </span>
        </div>
      </div>
    </div>
...
.bgImg {
  background-image: url("../RC1_0004-1024x683.jpg");
  width: 100%;
  height: 100vh;
